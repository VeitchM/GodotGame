...
func initialize():
	...
	serverListener.sRemoveServer.connect(self._removeServer) #Logica de bajo nivel, la interfaz de red de Server Listener
	serverListener.sNewServer.connect(self._newServer)
...
func _newServer(gameInfo):
	var server : BoxContainer = load(Repository.Rutes["Servers"]).instantiate()
	...
	server.sJoinServerconnect(get_node("../")._joinServer) #Se conectara con el control del menu de lobby
